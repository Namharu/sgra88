...
function clip(r,words){const clips=r.clips; return clips.find(c=>words.some(w=>c.name.toLowerCase().includes(w)))||clips[0]}
...
function attack(r,dt){r.attackCd-=dt;if(r.attackCd>0||r.stun>0||r.finished)return;const targets=racers.filter(t=>!t.finished&&Math.abs(t.lane-r.lane)===1&&Math.abs(t.z-r.z)<1.65);if(!targets.length){r.attackCd=.2;return}if(Math.random()>.48){r.attackCd=.4;return}const t=targets[Math.floor(Math.random()*targets.length)];r.attackCd=4+Math.random()*3;t.stun=.65;t.speed*=.42;play(r,['punch','attack','headbutt'],true);play(t,['hit'],true);r.root.rotation.z=(t.lane-r.lane)*-.2;burst(t.root.position,0xffffff);announce(`${r.name} → ${t.name} 옆 트랙 공격!`)}
function updateRace(dt){const elapsed=(performance.now()-startedAt)/1000;timerEl.textContent=elapsed.toFixed(1);let leaderZ=8;
 racers.forEach(r=>{if(r.finished)return;attack(r,dt);r.stun=Math.max(0,r.stun-dt);r.boost=Math.max(0,r.boost-dt);if(!r.boost&&Math.random()<dt*.035){r.boost=1.7;announce(`${r.name} 부스터!`)}const target=r.stun?1.4:r.base*(r.boost?1.65:1);r.speed=THREE.MathUtils.lerp(r.speed,target,dt*3.2);r.z-=r.speed*dt;r.root.position.z=r.z;
...
function animate(){requestAnimationFrame(animate);const dt=Math.min(clock.getDelta(),.04);racers.forEach(r=>r.mixer.update(dt));effects.forEach((e,i)=>{e.life-=dt;e.o.position.addScaledVector(e.v,dt);e.v.y-=7*dt;e.o.scale.setScalar(Math.max(0,e.life));if(e.life<=0){scene.remove(e.o);effects.splice(i,1)}});if(running)updateRace(dt);renderer.render(scene,camera)}
